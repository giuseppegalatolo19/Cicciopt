...
export function ServicesPage() {
  const catalog = usePublicServices();
  const content=usePublicContent({"services.page":{title:"Un allenamento che parla di te.",body:"Ogni servizio è un punto di partenza. Il percorso viene adattato a obiettivi, esperienza e disponibilità."}})["services.page"];
  return <><PageHero eyebrow="Servizi" title={<>{content.title}</>} body={content.body} /><section className="section"><Container><div className="service-grid all-services">{catalog.map((service, index) => <ServiceCard key={service.slug} service={service} index={index} />)}</div></Container></section><ContactBand /></>;
}
...
export function ContactForm() {
  const catalog=usePublicServices();
  const publicContact=usePublicContent({"contact.main":{title:"Raccontami da dove vuoi partire.",body:"Compila il modulo: riceverai una conferma e ti ricontatterò per capire insieme il passo successivo.",phone:"+39 000 000 0000",email:"ciao@francescocrivello.it",instagram:"@francescocrivello.pt"}})["contact.main"];
...
export function LegalPage({ type }: { type: "privacy" | "cookie" | "terms" }) {
  ...
  const key=type==="privacy"?"legal.privacy":type==="terms"?"legal.terms":"";const legal=usePublicContent(key?{[key]:{title:titles[type],intro:"Bozza professionale da verificare con il consulente legale.",body:""}}:{});const custom=key?legal[key]:null;
  return <><PageHero eyebrow="Informazioni legali" title={<>{custom?.title||titles[type]}</>} body={custom?.intro||"Bozza professionale da far verificare e completare al titolare del trattamento e al consulente legale prima della pubblicazione."} /><section className="section"><Container className="legal-copy">{custom?.body&&<div className="legal-custom-copy"><p>{custom.body}</p></div>}<div className="legal-notice"><ShieldCheck /><p><strong>Documento da verificare.</strong> Il testo pubblicato deve essere validato dal titolare e dal consulente legale.</p></div>{type === "privacy" ? <><h2>1. Titolare del trattamento</h2><p>Francesco Crivello, [indirizzo e contatti completi da inserire].</p><h2>2. Dati trattati e finalità</h2><p>Dati di contatto per rispondere alle richieste; dati dell’account per erogare i servizi; dati relativi alla salute, solo con consenso esplicito, per personalizzare in sicurezza il percorso.</p><h2>3. Base giuridica e conservazione</h2><p>Consenso, esecuzione di misure precontrattuali e contratto. I tempi di conservazione sono definiti nel registro dei trattamenti e limitati al necessario.</p><h2>4. Diritti</h2><p>Accesso, rettifica, cancellazione, limitazione, portabilità e revoca del consenso possono essere richiesti dall’area personale o ai contatti indicati.</p></> : type === "cookie" ? <><h2>Cookie necessari</h2><p>Servono per sessione, sicurezza, preferenze privacy e funzionamento dei moduli. Non possono essere disattivati dal banner.</p><h2>Misurazione facoltativa</h2><p>Eventuali analytics sono caricati solo dopo consenso. Nessun dato sanitario viene inviato a sistemi pubblicitari o tracciamento.</p><h2>Gestione preferenze</h2><p>Puoi riaprire il pannello dal link “Gestisci cookie” nel footer.</p></> : <><h2>Prenotazioni</h2><p>Le sessioni possono essere modificate o annullate dall’area cliente fino a 24 ore prima, salvo condizioni diverse indicate nel servizio.</p><h2>Ritardi e assenze</h2><p>Le regole definitive, inclusi eventuali addebiti, devono essere accettate separatamente e restano sempre consultabili.</p><h2>Salute e responsabilità</h2><p>L’attività di personal training non sostituisce assistenza medica o fisioterapica. Può essere richiesto un certificato medico valido.</p></>}</Container></section></>;
}
