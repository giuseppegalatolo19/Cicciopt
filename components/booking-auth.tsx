...
export function BookingConfirmation({
  ...
  const [copy,setCopy]=useState({confirmation:"Richiesta inviata correttamente.",account_invite:"Crea un account per seguire lo stato della prenotazione, modificare i tuoi dati e gestire più comodamente le richieste future."});
  useEffect(()=>{(createClient().from("site_content") as any).select("value").eq("content_key","forms.booking").eq("is_published",true).maybeSingle().then(({data}:{data:any})=>{if(data?.value)setCopy(current=>({...current,...data.value}))})},[]);
  ...
  return <section className="confirmation-page"><Container><div className="confirmation-icon"><CheckCircle2 size={40} /></div><Status tone="success">Prenotazione registrata</Status><h1>{copy.confirmation}</h1><p>{notificationCopy}</p><div className="confirmation-card"><div><small>Servizio</small><strong>{service}</strong></div><div><small>Data e ora</small><strong>{day}{time ? `, ${time}` : ""}</strong></div><div><small>Modalità</small><strong>{mode}</strong></div>{bookingCode && <div><small>Codice prenotazione</small><strong>{bookingCode}</strong></div>}</div>{claimAvailable&&<div className="account-invite"><h2>Gestisci tutto dalla tua area</h2><p>{copy.account_invite}</p><div className="confirmation-actions"><Link className="button button-primary" href="/registrazione?booking=1&next=/cliente/calendario?claim=1" onClick={prepareAccount}>Crea un account</Link><Link className="button button-secondary" href="/login?booking=1&next=/cliente/calendario?claim=1" onClick={prepareAccount}>Ho già un account</Link></div></div>}<div className="confirmation-actions"><Button href="/" variant="secondary">Continua senza account</Button></div><p className="small-note">La creazione dell’account è facoltativa. La richiesta resta registrata anche se chiudi questa pagina.</p></Container></section>;
}
