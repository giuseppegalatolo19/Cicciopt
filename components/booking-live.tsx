...
type BookingResult = { bookingId?: string; notificationStatus?: "sent" | "failed" | "not_configured" | "skipped" };
...
export default function BookingPage() {
  ...
  async function confirm(event: FormEvent<HTMLFormElement>) {
    ...
    setConfirmed({ bookingId: payload.bookingId, notificationStatus: payload.notificationStatus || "skipped" });
  ...
  if (confirmed) return <BookingConfirmation service={selectedService?.name} mode={selectedLocation?.name} day={dateText} time={timeText} bookingCode={confirmed.bookingId?.slice(0, 8).toUpperCase()} notificationStatus={confirmed.notificationStatus} />;
