...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: client, error }, { data: appointments }, { data: anamneses }, { data: documents }, { data: consents }, { data: notes }] = await Promise.all([
    (supabase.from("clients") as any).select("id,profile_id,first_name,last_name,email,phone,birth_date,admin_notes,account_status,primary_goal,next_action,created_at,profiles(id,email,first_name,last_name,phone)").eq("id", id).single(),
  ...
  const normalizedClient = { ...client, profiles: client.profiles ?? { id:null, email:client.email, first_name:client.first_name, last_name:client.last_name, phone:client.phone } };
  return NextResponse.json({ client: normalizedClient, appointments, anamnesis: anamneses?.[0] ?? null, documents, consents, notes });
}
